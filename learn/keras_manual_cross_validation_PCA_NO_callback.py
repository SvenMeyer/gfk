...
tensorflow back crash with: PCA = False , X_data.shape[1] >= 2000 (theano is ok!)
'''
...
# X_data = df.iloc[:,2:2050].values.astype('float32')


if NORM: # scale sum of ROW (user) to 1
    norm_scale = preprocessing.Normalizer(copy=False).fit(X_data)
    X_data = norm_scale.transform(X_data)
else:    # scale sum of COLUMN (feature) to 1
...
with tf.device("/gpu:0"):
    ...
    for train, validate in kfold.split(X, Y):
      ...
      # create model
        ...
        # Fit the model
        history = model.fit(X[train], Y[train], epochs=200, batch_size=10, verbose=1) # , callbacks=callbacks_list)
        print("DONE in ", (time.time() - start), "sec")
        # evaluate the model
        ...
        print("cohen_kappa_score = ", cohen_kappa_score(Y[validate], Y_predict))
        print("accuracy_score    = ",    accuracy_score(Y[validate], Y_predict))
        i += 1
