...
checkpoint = keras.callbacks.ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='auto', period=1)
# early stopp callback
# callbackEarlyStopp = keras.callbacks.EarlyStoppingByLossVal(monitor='val_loss', value=0.00001, verbose=1),
callbackEarlyStopp = keras.callbacks.EarlyStopping(monitor='val_loss', patience=2, verbose=0)

callbacks_list = [tbCallback, checkpoint, callbackEarlyStopp]
...
estimators.append(('mlp', KerasClassifier(build_fn=create_model, nb_epoch=300, batch_size=16, verbose=0))) # , callbacks=calls
